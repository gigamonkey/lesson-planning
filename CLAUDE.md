...
| `import_objectives.py` | Imports raw objectives into a course's pool, interning by text (idempotent; `parse_coverage` + `upsert`). Plain-text (pool only) or TSV (`objective`/`text`, optional `hierarchy_id`/`node_id` coverage edge, optional `uuid`). No default coverage target — a row's hierarchy is its `hierarchy_id` (bare slug; course from context) or `--hierarchy`, else pool-only. `--replace` re-seeds. |
