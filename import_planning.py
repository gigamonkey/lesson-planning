...
the columns) and replaces that table's rows. `nodes` is NOT touched -- it is
regenerated from the hierarchy markdown with load_nodes.py. Together those three
let you rebuild a database from scratch from version-controlled inputs:
...
# keys aren't enforced, but this keeps the order sensible.) `nodes` is excluded.
TABLES = ["objectives", "units", "lessons", "course_objectives", "coverage"]
